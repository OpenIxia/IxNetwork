...
#              in async mode and get session stats                             #
...
puts "Start session with async mode ..."
set fport_control_status [::ixia::fc_control\
        -action start\
        -action_mode async\
        -handle $fc_fport_handle]
if {[keylget fport_control_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget fport_control_status log]"
    return 0
}
set client_control_status [::ixia::fc_control\
        -action start\
        -action_mode async\
        -port_handle $fc_client_port_handle]

if {[keylget client_control_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget client_control_status log]"
    return 0
}
puts "Wait for the sessions are established..."
after 20000
#===============================GET SESSION STATISTICS=======================#
puts "Getting session statistics ..."
set stats_session_fport [::ixia::fc_fport_stats\
        -mode session\
        -port_handle $fc_fport_port_handle]

if {[keylget stats_session_fport status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget stats_session_fport log]"
    return 0
}
set stats_session_client [::ixia::fc_client_stats\
        -mode session\
        -port_handle $fc_client_port_handle]
        
if {[keylget stats_session_client status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget stats_session_client log]"
    return 0
}
set fport_session_id [lindex [keylkeys stats_session_fport session] 0]
set client_session_id [lindex [keylkeys stats_session_client session] 1]
set session_name_fport [keylget stats_session_fport session.$fport_session_id.session_name]
set flogi_ls_acc_tx_fport [keylget stats_session_fport session.$fport_session_id.flogi_ls_acc_tx]
set fdisc_ls_acc_tx_fport [keylget stats_session_fport session.$fport_session_id.fdisc_ls_acc_tx]
set plogi_ls_acc_tx_fport [keylget stats_session_fport session.$fport_session_id.plogi_ls_acc_tx]
set session_name_client [keylget stats_session_client session.$client_session_id.session_name]
set flogi_ls_acc_rx_client [keylget stats_session_client session.$client_session_id.flogi_ls_acc_rx]
set fdisc_ls_acc_rx_client [keylget stats_session_client session.$client_session_id.fdisc_ls_acc_rx]
set plogi_ls_acc_rx_client [keylget stats_session_client session.$client_session_id.plogi_ls_acc_rx]
puts "fport session id   : $fport_session_id"
puts "fport session name : $session_name_fport"
...
puts "client session id : $client_session_id"
puts "client session name : $session_name_client"
