...
#                                                                              #
# Description:                                                                 #
#    This sample configures a traffic item with two vlan stacks and then       #
#    links one field to another, before running the traffic.                   #
#                                                                              #
# Module:                                                                      #
...
if {[keylget traffic_result status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget traffic_result log]"
    return 0
}
...
set headers [keylget traffic_result $current_config_element.headers]
set stream_id [lindex $headers 1]
...
if {[keylget traffic_status_2 status] != $::SUCCESS} {
  puts "FAIL - $test_name - [keylget traffic_status_2 log]"
  return 0
}

set traffic_result [::ixia::traffic_config     \
    -mode          get_available_fields_for_link  \
...
keylprint traffic_result
set stack_link [keylget traffic_result available_fields_for_link]

set traffic_status_4 [::ixia::traffic_config \
    -mode modify \
    -stream_id $current_config_element \
    -field_linked [lindex $stack_link 0]\
    -field_linked_to [lindex $stack_link 1]\
    ]
if {[keylget traffic_status_4 status] != $::SUCCESS} {
    puts "ERROR - $test_name - [keylget traffic_status_4 log]"
    return 0
}
