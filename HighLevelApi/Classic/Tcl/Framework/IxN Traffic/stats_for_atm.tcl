...
################################################################################
# set specific_encapsulation_list {
...
#     VccMuxPPPoA                 "VC Multiplexed PPP"                        \ <- probleme !
...
################################################################################
foreach {pgid_encapsulation encapsulation_alias} $specific_encapsulation_list {
    puts "===== Running for Encapsulation: $encapsulation_alias"
    after 2000
    foreach {current_pgid_mode mode_alias} $pgid_modes_list {
        set pgid_mode $current_pgid_mode
        puts "***** Running for PGID mode: $mode_alias"
        set logid [createLogHere "${pgid_encapsulation}-${pgid_mode}"]
        ################################################################################
        ...
        for {set i 0} {$i < 5} {incr i} {
            ...
            lappend pgid_encap           $pgid_encapsulation
        }
        for {set i 0} {$i < 5} {incr i} {
