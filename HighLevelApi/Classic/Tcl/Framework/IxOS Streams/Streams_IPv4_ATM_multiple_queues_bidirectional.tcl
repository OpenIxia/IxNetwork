...
#                                                                              #
# Description:                                                                 #
#    This sample configures ATM interfaces and creates streams.                #
#                                                                              #
# Module:                                                                      #
...
################################################################################
# Configure first interface in the test 
################################################################################
...
for {set i 1} {$i <= 2} {incr i} {
    ...
    lappend src_mac_addr_list                   0000.dead.00[format "%02x" $i]
...
################################################################################
# Configure second interface in the test 
################################################################################
set port_handle_list                    ""
set intf_mode_list                      ""
set intf_ip_addr_list                   ""
...
for {set i 1} {$i <= 2} {incr i} {
    ...
    lappend intf_mode_list                      atm
    lappend intf_ip_addr_list                   12.1.$i.1
    lappend gateway_list                        12.1.$i.2
    lappend netmask_list                        255.255.255.0
    lappend src_mac_addr_list                   0000.beef.00[format "%02x" $i]
    lappend atm_enable_coset_list               0
    lappend atm_enable_pattern_matching_list    0
    lappend atm_filler_cell_list                idle
    lappend atm_interface_type_list             nni
...
set traffic_status [::ixia::traffic_config                                  \
        ...
        -mac_src                            0000.dead.0001                  \
        -mac_src2                           0000.beef.0001                  \
        ]
if {[keylget traffic_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget traffic_status log]"
    return
...
set traffic_status [::ixia::traffic_config                                  \
        ...
        -l2_encap                           atm_vc_mux_802.3snap            \
        -mac_dst_mode                       discovery                       \
        -mac_src                            0000.dead.0002                  \
        -mac_src2                           0000.beef.0002                  \
        ]
if {[keylget traffic_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget traffic_status log]"
    return
