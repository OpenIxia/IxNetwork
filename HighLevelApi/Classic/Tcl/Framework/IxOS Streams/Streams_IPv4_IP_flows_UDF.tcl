...
#                                                                              #
# Description:                                                                 #
#    This sample creates an IPv4 stream. Packet group id and source IP address #
#    are incremented using UDFs. Flow stats are saved to file                  #
#    ip_flows_test_results.txt                                                 #
#                                                                              #
# Module:                                                                      #
...
# Add the stream ID to the list for future purpose
set stream_index [keylget traffic_status1 stream_id]

# you should use a b2b setup else following commands will fail
set interface_status [::ixia::interface_config  \
        -port_handle     $port_handle_tx        \
        -arp_send_req    1                      ]
if {[keylget interface_status status] != $::SUCCESS} {
...
if [catch {open $file_name w} file_id] {
    puts "ERROR::Cannot open file $file_id!"
    return -1
}

puts $file_id "\n"
puts $file_id "+-  ------------------------------------------------------------+"
puts $file_id "+                       Statistic Results                       +"
puts $file_id "+---------------------------------------------------------------+"
puts $file_id "+ Time            : [clock format [clock seconds]]"
puts $file_id "+ Number of Flows : $flow_count"
puts $file_id "+ Note            : Latency values are in nsec"
puts $file_id "+---------------------------------------------------------------+"
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        Flow Packets Minimum Maximum Average Rate]
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        Number Received Latency Latency Latency BPS]
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        "--------" "--------" "--------" "--------" "--------" "--------"]

for {set k 1} {$k <= $flow_count} {incr k} {



    puts $file_id [format "%8d  %8d  %8d  %8d  %8d  %8d"                        \
            $k \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.pkt_count.$k] \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.min_latency.$k] \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.max_latency.$k] \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.avg_latency.$k] \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.bit_rate.$k] ]
