...
#                                                                              #
# Description:                                                                 #
#    This sample creates an IPv4 VLAN stream with one UDF.                     #
#                                                                              #
# Module:                                                                      #
...
set file_name "vlan_streams_test_results.txt"
...
set traffic_status  [::ixia::traffic_config          \
        ...
        -udf3_counter_repeat_count $vlan_number    \
        -udf3_counter_step         1               \
...
puts $file_id "+ Number of VLANs   : $vlan_number"
puts $file_id "+ Note              : Latency values are in nsec"
puts $file_id "+---------------------------------------------------------------+"
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        VLAN Packets Minimum Maximum Average Rate]
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        Number Received Latency Latency Latency BPS]
puts $file_id [format "%8s  %8s  %8s  %8s  %8s  %8s" \
        "--------" "--------" "--------" "--------" "--------" "--------"]

for {set k 1} {$k <= $number_of_pgids} {incr k} {

    puts $file_id [format "%8d  %8d  %8d  %8d  %8d  %8d" $k       \
            [keylget pgid_statistics_list $port_handle_rx.pgid.rx.pkt_count.$k] \
            [keylget pgid_statistics_list \
        $port_handle_rx.pgid.rx.min_latency.$k] \
            [keylget pgid_statistics_list \
        $port_handle_rx.pgid.rx.max_latency.$k] \
            [keylget pgid_statistics_list \
        $port_handle_rx.pgid.rx.avg_latency.$k] \
