...
#                                                                              #
# Description:                                                                 #
#    This sample creates an IPv4 VLAN stream. Packet group id is incremented   #
#    using an UDF. Latency statistics are displayed for each packet group id.  #
#                                                                              #
# Module:                                                                      #
...
puts  "+ Number of PacketGroups : $number_of_pgids"
puts  "+ Number of Latency Bins : $number_of_bins"
puts  "+ Note                   : Latency values are in nsec"
puts  "+---------------------------------------------------------------+"
puts  [format "%8s  %8s  %15s  %15s  %8s  %8s  %8s" \
        PGID Bin# FirstTS LastTS MaxLat MinLat TotalPackets]

 
for {set pgid_index 1} {$pgid_index <= $number_of_pgids} {incr pgid_index} { 
    for {set l 1} {$l <= $number_of_bins} {incr l} {

       puts  [format "%8d  %8d  %15.1f  %15.1f  %8.1f  %8.1f  %8d" $pgid_index   $l \
            [keylget pgid_statistics_list \
       $port_handle_rx.pgid.$pgid_index.rx.latency_bin.$l.first_tstamp] \
            [keylget pgid_statistics_list \
       $port_handle_rx.pgid.$pgid_index.rx.latency_bin.$l.last_tstamp] \
            [keylget pgid_statistics_list \
       $port_handle_rx.pgid.$pgid_index.rx.latency_bin.$l.max] \
            [keylget pgid_statistics_list \
       $port_handle_rx.pgid.$pgid_index.rx.latency_bin.$l.min] \
            [keylget pgid_statistics_list \
       $port_handle_rx.pgid.$pgid_index.rx.latency_bin.$l.total_pkts] \
            [keylget pgid_statistics_list ]]
    }
}
