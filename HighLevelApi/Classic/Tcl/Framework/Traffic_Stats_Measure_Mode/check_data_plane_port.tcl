...
#    The stats is calculated using mode data_plane_port and with measure mode  #
#    instantaneous, cumulative and mixed                                       #
# Module:                                                                      #
...
set ixn_cfg                 [file join $test_name_folder check_data_plane_port.ixncfg]
...
set wait 10
puts "waiting for ${wait}s to gather statistics"
...
for {set try 0} {$try < 30} {incr try} {
    set res_live_instantaneous [ixia::traffic_stats  \
        -mode data_plane_port                   \
        -measure_mode instantaneous             \
    ]
    if {[keylget res_live_instantaneous status] != $::SUCCESS} {
        error "traffic_stats run failed: $res_live_instantaneous"
    }
    if {![keylget res_live_instantaneous waiting_for_stats]} {
        break
    }
    after 1000
}
...
set wait 20
puts "waiting for ${wait}s to finish sending the traffic"
after [expr 1000 * $wait]
...
for {set try 0} {$try < 30} {incr try} {
    ...
    if {[keylget res_instantaneous status] != $::SUCCESS} {
        error "traffic_stats run failed: $res_instantaneous"
    }
...
for {set try 0} {$try < 30} {incr try} {
    ...
    if {[keylget res_cumulative status] != $::SUCCESS} {
        error "traffic_stats run failed: $res_cumulative"
    }
...
for {set try 0} {$try < 30} {incr try} {
    set res_mixed [ixia::traffic_stats          \
        -mode data_plane_port                   \
        -measure_mode mixed                     \
    ]
    if {[keylget res_mixed status] != $::SUCCESS} {
        error "traffic_stats run failed: $res_mixed"
    }
    if {![keylget res_mixed waiting_for_stats]} {
        break
    }
    after 1000
}
