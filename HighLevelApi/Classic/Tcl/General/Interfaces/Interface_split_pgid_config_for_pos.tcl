...
set ::cfg::card_type     pos
set ::cfg::setup_type    b2b                       ;# CHOICES b2b dut
set ::cfg::num_ports     2
set ::cfg::l1_intf_mode  pos_ppp                   ;# CHOICES atm 
...
################################################################################
array set specific_intf_mode_array {
    pos_hdlc                    "CISCO HDLC"
    pos_ppp                     "PPP"
...
################################################################################
set error_counter 0
foreach current_intf_mode [array names specific_intf_mode_array] {
    puts "\n======================== Interface mode = $current_intf_mode ========================"
    set intf_mode $current_intf_mode
        ...
        foreach port_identifier [ixNet getL [ixNet getRoot] vport] {
            ...
            foreach current_component [ixNet getL $traffic_port component] {
                ...
                if {$pgid_mode != "split"} {
                    ...
                    if {$particle == $specific_intf_mode_array($current_intf_mode)} {
                        puts "  Encapsulation - $particle - Ok."
                    } else {
                        puts "! Encapsulation is $particle (should be $specific_intf_mode_array($current_intf_mode))"
                        incr error_counter
                    }
                }
                #-----------------------------------------------------------------------
