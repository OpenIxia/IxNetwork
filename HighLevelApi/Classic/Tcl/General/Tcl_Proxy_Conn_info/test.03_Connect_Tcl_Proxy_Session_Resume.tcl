...
#                                                                              #
# Description:                                                                 #
#    This is a sample script for session resume functionality with             #
#    IxNetwork Tcl Proxy Server and displays the proxy connection information. #
# Module:                                                                      #
...
set ixn_cfg                                     [file join $test_name_folder test.sample_config.ixncfg]
...
################################################################################
# Store the username to which the current session is connected
################################################################################
set username [keylget connect_status connection.tcl_proxy_username]

################################################################################
# START - DisConnect to the chassis
################################################################################
set disconnect_status [::ixia::cleanup_session]
if {[keylget disconnect_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget disconnect_status log]"
    return 0
}
################################################################################
...
################################################################################
# START - Connect with the previous username to restore the session
################################################################################

set connect_status_2 [::ixia::connect                           \
            -mode                   connect                     \
            -ixnetwork_tcl_server   $ixnetwork_tcl_server       \
            -tcl_proxy_username     $username                   \
            -interactive            1                           ]

if {[keylget connect_status_2 status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget connect_status_2 log]"
    return 0
}

###############################################################################
# END - Connect to the chassis 2
###############################################################################
