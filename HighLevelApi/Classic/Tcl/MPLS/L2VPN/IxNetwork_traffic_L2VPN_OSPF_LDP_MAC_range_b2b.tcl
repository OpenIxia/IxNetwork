...
set mac_range_count_list       [list 5          5]
################################################################################
...
foreach port [list $port_0 $port_1] {
    ...
    set ldp_route_status [::ixia::emulation_ldp_route_config                   \
            ...
            -fec_vc_mac_range_count       [lindex $mac_range_count_list $index]\
            -fec_vc_mac_range_repeat_mac  0                                    \
            -fec_vc_mac_range_same_vlan   0                                    \
            -fec_vc_mac_range_vlan_enable 0                                    \
            -fec_vc_mac_range_start       0000.[format "%04x" [join [split $port "/"] ""]].0000 \
            -provisioning_model           manual_configuration                 \
            ]
    if {[keylget ldp_route_status status] != $::SUCCESS} {
...
if {[keylget ldp_control_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget ldp_control_status log]"
    return
}
