...
proc script_increment_bytes { macAdd amount {numBytes 6} {separator .}} {
    ...
    set retValue [set b[expr $i - 1]]
    for {set j [expr $i - 2]} {$j > -1} {incr j -1} {
        append retValue "${separator}[set b$j]"
    }
    return $retValue
}
...
# Set Provider parameters (used to configure ISIS and LDP routers)
set p_count              2
...
# Set PE parameters (used to configure ISIS and LDP routers)
set pe_count              3
...
set pe_intf_addr_step1    0.0.1.0
set pe_intf_addr_step2    10.10.10.10
set pe_intf_prefix_len    24
set pe_intf_mask          255.255.255.0
set pe_isis_type_list     [list route grid]
set pe_isis_type          [lindex $pe_isis_type_list 0]
...
foreach {port_ce} $port_ce_handle {port_pe} $port_pe_handle {
    ...
    for {set i -1} {$i < $index} {incr i} {
        ...
        set ce_tx_mac_start_p     [script_increment_bytes  \
                $ce_tx_mac_start_p     $ce_tx_mac_step2]
        
        set ce_rx_mac_start_p     [script_increment_bytes  \
    ...
    # Configure ISIS Provider Router
    set isis_router_status [::ixia::emulation_isis_config             \
            ...
            -intf_ip_prefix_length          $p_ip_prefix_len        \
            -count                                1                       \
            -intf_metric                    0                       \
            -routing_level                  L1L2                    \
            -te_enable                      0                       \
            ...
            -system_id                      0x111111111[expr $index + 3]00 \
            -system_id_step                 0                       \
            ]
    ...
    set     isis_router_provider [keylget isis_router_status handle]
    lappend isis_router_list     [keylget isis_router_status handle]
    ...
    for {set i 0} {$i < $pe_count} {incr i} {
        # Configure ISIS PE
        if {$pe_isis_type == "route"} {
            # As route
            set route_config_status [::ixia::emulation_isis_topology_route_config \
                    -mode                   create                  \
                    -handle                 $isis_router_provider   \
                    -type                   stub                    \
                    -ip_version             4                       \
                    -stub_ip_start          $pe_ip_addr_temp        \
                    -stub_ip_pfx_len        $pe_ip_prefix_len       \
                    -stub_count             1                       \
                    -stub_metric            1                       \
                    -stub_up_down_bit       0                       ]
            ...
            set route_config_status [::ixia::emulation_isis_topology_route_config \
                    ...
                    -grid_router_ip_pfx_len  $pe_ip_prefix_len        \
    ...
    incr stream_tx_sgn_temp  $stream_tx_sgn_step
}

# Start ISIS protocol
set isis_control_status [::ixia::emulation_isis_control \
        -port_handle           $port_pe_handle        \
        -mode                  start                  ]

if {[keylget isis_control_status status] != $::SUCCESS} {
    return "FAIL - $test_name - [keylget isis_control_status log]"
}
...
# Wait 100 seconds for the ISIS and LDP to learn routes and labels
after 100000
