...
# !
# router ospf 401 vrf vpn401
#  redistribute bgp 1 subnets
#  network 22.1.1.0 0.0.0.255 area 0
# !
# router ospf 402 vrf vpn402
#  redistribute bgp 1 subnets
#  network 22.1.2.0 0.0.0.255 area 0
# !
# router ospf 403 vrf vpn403
#  redistribute bgp 1 subnets
#  network 22.1.3.0 0.0.0.255 area 0
# !
# router ospf 404 vrf vpn404
#  redistribute bgp 1 subnets
#  network 22.1.4.0 0.0.0.255 area 0
# !
# router ospf 405 vrf vpn405
#  redistribute bgp 1 subnets
#  network 22.1.5.0 0.0.0.255 area 0
# !
# router isis PE
...
#  redistribute ospf 401 vrf vpn401 match internal external 1 external 2
#  no synchronization
#  exit-address-family
#  !
#  address-family ipv4 vrf vpn402
#  redistribute ospf 402 vrf vpn402 match internal external 1 external 2
#  no synchronization
#  exit-address-family
#  !
#  address-family ipv4 vrf vpn403
#  redistribute ospf 403 vrf vpn403 match internal external 1 external 2
#  no synchronization
#  exit-address-family
#  !
#  address-family ipv4 vrf vpn404
#  redistribute ospf 404 vrf vpn404 match internal external 1 external 2
#  no synchronization
#  exit-address-family
#  !
#  address-family ipv4 vrf vpn405
#  redistribute ospf 405 vrf vpn405 match internal external 1 external 2
#  no synchronization
#  exit-address-family
# !
# end
...
################################################################################
# Configure OSPFv2 emulated routers on the customer port                       #
################################################################################
...
foreach {ospf_ce_handle} $ospf_ce_handles {
    set ospf_route_status [::ixia::emulation_ospf_topology_route_config     \
            ...
            -type                       summary_routes                      \
            -summary_prefix_start       $c_site1_temp                       \
            -summary_prefix_length      $c_prefix1_len                      \
    ...
    set ospf_route_status [::ixia::emulation_ospf_topology_route_config     \
            ...
            -handle                     $ospf_ce_handle                     \
            -type                       summary_routes                      \
            -summary_prefix_start       $c_site2_temp                       \
            -summary_prefix_length      $c_prefix2_len                      \
...
################################################################################
# Start OSPFv2 on the client port                                              #
################################################################################
