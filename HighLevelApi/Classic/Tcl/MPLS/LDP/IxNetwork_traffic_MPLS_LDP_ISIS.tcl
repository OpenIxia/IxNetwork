...
################################################################################
# Configure an LDP emulated router on each port                                #
################################################################################
set ldp_router_handle_list [list]
set local_ip_address_temp   $local_ip_address
set gateway_ip_address_temp $gateway_ip_address
for {set i 0} {$i < $port_count} {incr i} {
    ...
    if {[keylget ldp_router_status status] != $::SUCCESS} {
        return "FAIL - [keylget ldp_router_status log]"
    }
    lappend ldp_router_handle_list [keylget ldp_router_status handle]
...
################################################################################
#  Advertise a FEC class from each LDP router                                  #
################################################################################
set route_range_ip_address_temp   $route_range_ip_address
for {set i 0} {$i < $port_count} {incr i} {
    set ldp_router_handle [lindex $ldp_router_handle_list $i]
    set route_config_status [::ixia::emulation_ldp_route_config               \
            -mode                       create                              \
            -handle                     $ldp_router_handle                  \
            -fec_type                   ipv4_prefix                         \
            -fec_ip_prefix_start        $route_range_ip_address_temp        \
            -fec_ip_prefix_length       21                                  \
            -num_lsps                   5                                   \
            -label_value_start          [expr $i + 1]00                     \
...
set traffic_status [::ixia::traffic_config                                  \
        ...
        -name                   "MPLS_LDP_Traffic"                          \
        -src_dest_mesh          fully                                       \
        -route_mesh             one_to_one                                  \
        -circuit_type           mpls                                        \
        -circuit_endpoint_type  ipv4                                        \
        -emulation_src_handle   [lindex $ldp_router_handle_list 0]          \
        -emulation_dst_handle   [lindex $ldp_router_handle_list 1]          \
