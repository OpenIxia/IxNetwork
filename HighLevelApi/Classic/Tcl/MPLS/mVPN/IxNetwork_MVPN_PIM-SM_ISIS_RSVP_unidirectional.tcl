...
set isis_topology_status [::ixia::emulation_isis_topology_route_config \
        -mode                           create                  \
        -handle                         $provider_isis_handle   \
        -type                           grid                    \
        -grid_col                       $pe_count               \
        -grid_row                       1                       \
        -grid_ip_start                  $pe_ip_addr_start       \
        -grid_ip_step                   $pe_ip_addr_step        \
]
if {[keylget isis_topology_status status] != $::SUCCESS} {
    return "FAIL - $test_name - [keylget isis_topology_status log]"
}
...
foreach {pe_pim_handle}  $pe_pim_handles {
    ...
    lappend source_router_list $pe_pim_handle
    if {$mvrf_unique == 0} {
        set mvpn_group_address      $mvpn_group_addr_start
        for {set i 0} {$i < $mvpn_num_sources} {incr i} {
            set mvpn_source_address   [script_increment_ipv4_net \
...
foreach {ce_pim_handle} $ce_pim_handles {
    ...
    lappend join_router_list $ce_pim_handle
    set mvpn_rp_address       [script_increment_ipv4_address \
            $mvpn_rp_address $mvpn_rp_addr_step]
