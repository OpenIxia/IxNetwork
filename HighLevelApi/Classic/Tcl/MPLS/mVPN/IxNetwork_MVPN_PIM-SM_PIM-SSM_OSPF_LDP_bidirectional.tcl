...
#    Customer Side Multicast Protocol - PIM/SSM                                
#                                                                              
# DUT configuration:                                                           
#                                                                              
# 
...
# no ip address
# 
# interface GigabitEthernet9/26.500
# description CE1
...
# no cdp enable
# 
# interface GigabitEthernet9/26.501
# description CE2
...
# 
# router ospf 2 vrf 500
# log-adjacency-changes
# network 21.21.25.0 0.0.0.255 area 0
# 
# router ospf 3 vrf 501
# log-adjacency-changes
# network 21.21.26.0 0.0.0.255 area 0
# 
# router bgp 1
...
# Add sources on CE with this params
set ce_mcast_source_addr_start    78.0.0.1
set ce_mcast_source_addr_step     0.0.1.0
set ce_mcast_source_prefix        32
set ce_mcast_num_sources          1

set ce_mcast_group_addr_start     229.0.0.1
...
################################################################################
# Create PIM-SSM MVPN sources on each PE
################################################################################
set mvpn_source_address  $mvpn_source_addr_start
set mvpn_group_address   $mvpn_group_addr_start
foreach {pe_pim_handle}  $pe_pim_handles {
    ...
    lappend pe_source_handles [keylget pim_config_status handle]
    if {$mvrf_unique == 0} {
        set mvpn_group_address      $mvpn_group_addr_start
        for {set i 0} {$i < $mvpn_num_sources} {incr i} {
            set mvpn_source_address   [script_increment_ipv4_net \
...
################################################################################
# Create PIM-SSM MVPN groups on each PE (for bidirectional traffic)
################################################################################
...
foreach {pe_pim_handle}  $pe_pim_handles {
    ...
    lappend ce_join_handles [keylget pim_config_status handle]
    
    set ce_mcast_group_address    $ce_mcast_group_addr_start
    set ce_mcast_source_address   $ce_mcast_source_addr_start
    
    set ce_mcast_source_address   [script_increment_ipv4_address \
            $ce_mcast_source_address 0.0.0.1]
}
...
foreach {ce_pim_handle} $ce_pim_handles {
    ...
    lappend pe_join_handles [keylget pim_config_status handle]
...
foreach {ce_pim_handle} $ce_pim_handles {
    ...
    lappend ce_source_handles [keylget pim_config_status handle]
...
set ospf_neighbor_status [::ixia::emulation_ospf_config \
        ...
        -vlan_id                    $ce_vlan_id_start  \
        -vlan_id_step               $ce_vlan_id_step   \
        -intf_prefix_length         $ce_ip_prefix      \
        -neighbor_intf_ip_addr      $ce_gw_addr_start  \
        -neighbor_intf_ip_addr_step $ce_gw_addr_step   \
        -router_id                  $ce_ip_addr_start  \
        -area_id                    0.0.0.0           \
        -area_id_step               0.0.0.0           \
        -area_type                  external-capable  \
        -authentication_mode        null              \
...
set ce_ospf_handles [keylget ospf_neighbor_status handle]

set ce_mcast_source_address $ce_mcast_source_addr_start
foreach {ce_ospf_handle} $ce_ospf_handles {
    ##############################################################
    # Create OSPFv2 CE Routes
    ##############################################################
    set route_config_status [::ixia::emulation_ospf_topology_route_config\
            -mode                     create                   \
            -handle                   $ce_ospf_handle          \
            -type                     summary_routes           \
            -summary_number_of_prefix $pe_count                \
            -summary_prefix_start     $ce_mcast_source_address \
...
foreach pe_source_list $pe_source_handles\
        ce_join_list   $ce_join_handles {
    foreach pe_source_item $pe_source_list   \
            ce_source_item $ce_source_handles\
            pe_join_item   $pe_join_handles  \
            ce_join_item   $ce_join_list {
        # create traffic from source emulated on PE (with behavior of CE source)
        set traffic_status [::ixia::traffic_config  \
            -mode                   create          \
            -traffic_generator      ixnetwork       \
            -emulation_src_handle   $pe_source_item \
            -emulation_dst_handle   $pe_join_item   \
            -frame_size             512             \
            -l3_protocol            ipv4            \
            -pkts_per_burst         2               \
            -rate_percent           1               \
        ...
        # create traffic from source emulated on CE
        set traffic_status [::ixia::traffic_config  \
            -mode                   create          \
            -traffic_generator      ixnetwork       \
            -emulation_src_handle   $ce_source_item \
            -emulation_dst_handle   $ce_join_item   \
            -frame_size             512             \
            -l3_protocol            ipv4            \
            -pkts_per_burst         2               \
            -rate_percent           1               \
