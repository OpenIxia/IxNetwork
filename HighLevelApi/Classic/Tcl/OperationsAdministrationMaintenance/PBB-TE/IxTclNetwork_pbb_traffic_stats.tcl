...
#    starts the protocol, creates traffic and retrieves per flow stats.        #
...
################################################################################
# START - IxN Traffic configuration
################################################################################
puts "\n### Traffic configure - Start"
...
set traffic_status [::ixia::traffic_config                                  \
        ...
        -traffic_generator           ixnetwork                              \
        -bidirectional               0                                      \
        -emulation_dst_handle        ${pbb_handles_1}                       \
        -emulation_src_handle        ${pbb_handles_2}                       \
        -circuit_endpoint_type       ethernet_vlan                          \
        -circuit_type                mac_in_mac                             \
        -track_by                    s_vlan                                 \
...
for {set i 0} {$i < [llength $port_list]} {incr i} {
    ...
    set flows [keylget flow_traffic_status \
            [subst $[subst port_$i]].flow]
    foreach flow [keylkeys flows] {
        set flow_key [keylget flow_traffic_status \
                [subst $[subst port_$i]].flow.$flow]
        puts "\tFlow $flow:"
        foreach {name key} $flow_results {
            puts "\t\t$name: [keylget flow_traffic_status\
                    [subst $[subst port_$i]].flow.$flow.$key]"
        }
    }
}
