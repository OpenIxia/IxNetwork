...
#    In the end some of route ranges are being deleted.                        #
#                                                                              #
# Module:                                                                      #
...
set router_handles [keylget eigrp_router_status router_handles]
################################################################################
...
set eigrp_router_status [::ixia::emulation_eigrp_route_config \
        ...
        -num_prefixes          10                             \
        -next_hop              25.0.0.1                       \
        -next_hop_outside_step 1.0.0.0                        \
        ]
if {[keylget eigrp_router_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget eigrp_router_status log]"
    return
}
set route_handles [keylget eigrp_router_status route_handles]
################################################################################
# Delete some EIGRP routes
################################################################################
set eigrp_router_status [::ixia::emulation_eigrp_route_config \
        -mode          delete                                 \
        -route_handle  [lrange $route_handles  1 3]           \
        ]
if {[keylget eigrp_router_status status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget eigrp_router_status log]"
    return
