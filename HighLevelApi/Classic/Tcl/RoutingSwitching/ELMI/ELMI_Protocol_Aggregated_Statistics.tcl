...
#    ELMI Aggregated statistics is also retreived                              #
# Module:                                                                      #
...
################################################################################
set elmi_info_aggr [::ixia::emulation_elmi_info                             \
        -port_handle                                $port_handle            ]
        
if {[keylget elmi_info_aggr status] != $::SUCCESS} {
    puts "FAIL - $test_name - [keylget elmi_info_aggr log]"
    return 0
}

foreach port $port_handle {
    puts "############### Stats for Port - $port ###############"    
    set aggr_stat_list [keylkeys elmi_info_aggr $port.aggregate]
    foreach stat $aggr_stat_list {
        if {[catch {set v [keylget elmi_info_aggr $port.aggregate.$stat]} err]} {
            puts [format {%20s %s} "FAIL - $port.aggregate.$stat -" "not present"]
            incr cfgErrors
        } else {
