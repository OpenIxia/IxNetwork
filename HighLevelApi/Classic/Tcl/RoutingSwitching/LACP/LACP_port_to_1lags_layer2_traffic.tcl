...
#    Layer 2 traffic is sent from a third ixia port to the lag.                #
#                                                                              #
# Module:                                                                      #
...
set port_list               [list 1/1 1/2 1/3]
set break_locks             1
...
set lacp_link_config_status [::ixia::emulation_lacp_link_config                 \
        ...
        -reset                                                                  \
        -lag_count                             1                                \
        -port_handle                           [lrange $port_handle 0 [expr     \
                                                    [llength $port_handle] - 2]]\
        -actor_key                             3                                \
...
set mac_dst                             [keylget lacp_cfg_info lag.[lindex [keylkeys lacp_cfg_info lag] 0].actor_system_id]
...
# UNSET TRAFFIC_CONFIG VARIABLES
...
################################################################################
set pgid_list ""
for {set j 1} {$j <= $num_pgids} {incr j} {
    lappend pgid_list $j
}
