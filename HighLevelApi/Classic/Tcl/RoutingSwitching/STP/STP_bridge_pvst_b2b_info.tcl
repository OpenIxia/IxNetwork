...
#   bridge created for each Ixia port. For each bridge, a VLAN object is       #
...
set stp_status [::ixia::emulation_stp_bridge_config \
        ...
        -cst_root_mac_address 0000.0000.0013        \
        -cst_root_path_cost   5                     \
        -bridge_mode          pvst                  \
        ]
...
set bridge_interface_handles [keylget stp_status bridge_interface_handles.$bridge_handle]

################################################################################
# Create a STP VLAN range for previous created STP bridge
################################################################################
...
set stp_status [::ixia::emulation_stp_bridge_config \
        ...
        -max_age              6000                  \
        -auto_pick_bridge_mac 1                     \
        -cst_root_mac_address 0000.0000.0014        \
        -cst_root_path_cost   5                     \
        -bridge_mode          pvst                  \
        ]
if {[keylget stp_status status] != $::SUCCESS} {
...
set bridge_interface_handles2 [keylget stp_status bridge_interface_handles.$bridge_handle2]
...
foreach stp_intf $bridge_interface_handles {
    ...
    set stat_list [keylget stp_info\
            $port_0.$bridge_handle.vlan_intf.$vlan_id.$stp_intf]
...
puts "BPDUs Rx: [keylget stp_info_stats $port_0.aggregate.pvst_bpdus_rx]"
puts "BPDUs Tx: [keylget stp_info_stats $port_0.aggregate.pvst_bpdus_tx]"
