...
def configDeviceGroupProtocolStack(deviceGroupObj, deviceGroupData):
    ...
    for ethernet in deviceGroupData['ethernet']:
        ...
        if 'networkGroup' in deviceGroupData:
            for networkGroup in deviceGroupData['networkGroup']:
...
def config(paramModuleName, connectToApiServer='windows'):
    global protocolObj

    for attr in ('stdin', 'stdout', 'stderr'):
        setattr(sys, attr, getattr(sys, '__%s__' %attr))

    print('--- paramFile:', paramModuleName)

    param = __import__(paramModuleName).params
    try:
